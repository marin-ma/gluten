...
import io.glutenproject.expression.{CheckOverflowTransformer, DecimalArithmeticExpressionTransformer, ExpressionTransformer, PromotePrecisionTransformer}
...
object DecimalArithmeticUtil {
  ...
  def getResultType(transformer: ExpressionTransformer): Option[DecimalType] = {
    transformer match {
      case PromotePrecisionTransformer(child, _) =>
        getResultType(child)
