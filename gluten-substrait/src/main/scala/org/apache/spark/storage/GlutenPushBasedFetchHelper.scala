 ...
 * multiple shuffle chunks where each shuffle chunk contains multiple shuffle blocks that belong to
 * the common reduce partition and were merged by the external shuffle service to that chunk.
 */
private class GlutenPushBasedFetchHelper(
   ...
   *
   * @param req
   *   [[ShuffleBlockFetcherIterator.FetchRequest]] that only contains requests to fetch metadata of
   *   push-merged blocks.
   */
   ...
   *
   * This initiates fetching fallback blocks for a push-merged block or a shuffle chunk that failed
   * to fetch. It makes a call to the map output tracker to get the list of original blocks for the
   * given push-merged block/shuffle chunk, split them into remote and local blocks, and process
   * them accordingly. It also updates the numberOfBlocksToFetch in the iterator as it processes
   * failed response and finds more push-merged requests to remote and again updates it with
   * additional requests for original blocks. The fallback happens when:
   *   1. There is an exception while creating shuffle chunks from push-merged-local shuffle block.
   *      See fetchLocalBlock. 2. There is a failure when fetching remote shuffle chunks. 3. There
   *      is a failure when processing SuccessFetchResult which is for a shuffle chunk (local or
   *      remote). 4. There is a zero-size buffer when processing SuccessFetchResult for a shuffle
   *      chunk (local or remote).
   */
