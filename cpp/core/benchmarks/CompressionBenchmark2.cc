...
using compressFunction =
    std::function<std::vector<uint8_t>(const std::vector<std::shared_ptr<arrow::Buffer>>&, int32_t)>;

std::vector<uint8_t> lz4CompressBuffer(
    const std::vector<std::shared_ptr<arrow::Buffer>>& buffers,
    int32_t compressionLevel) {
  std::vector<uint8_t> compressed;
  std::vector<uint8_t> uncompressed;
  auto totalBufferLen =
      std::accumulate(buffers.begin(), buffers.end(), 0, [](int64_t sum, const std::shared_ptr<arrow::Buffer>& buffer) {
        return sum + buffer->size();
      });
  uncompressed.resize(totalBufferLen);

  LZ4F_preferences_t prefs;
  memset(&prefs, 0, sizeof(prefs));
  prefs.compressionLevel = compressionLevel;
  auto maxCompressedLen = LZ4F_compressFrameBound(totalBufferLen, &prefs);
  compressed.reserve(maxCompressedLen);

  size_t currentPos = 0;
  for (const auto& buffer : buffers) {
    memcpy(uncompressed.data() + currentPos, buffer->data(), buffer->size());
    currentPos += buffer->size();
  }

  auto compressedSize = LZ4F_compressFrame(
      compressed.data(), (size_t)maxCompressedLen, uncompressed.data(), (size_t)totalBufferLen, &prefs);

  compressed.resize(compressedSize);
  return compressed;
}

std::vector<uint8_t> zstdCompressBuffer(
    const std::vector<std::shared_ptr<arrow::Buffer>>& buffers,
    int32_t compressionLevel) {
  std::vector<uint8_t> compressed;
  std::vector<uint8_t> uncompressed;
  ...
  auto compressedSize = ZSTD_compress(
      compressed.data(), (size_t)maxCompressedLen, uncompressed.data(), (size_t)totalBufferLen, compressionLevel);

  compressed.resize(compressedSize);
  return compressed;
}

std::vector<uint8_t> zstdCompressBuffers(
    const std::vector<std::shared_ptr<arrow::Buffer>>& buffers,
    int32_t compressionLevel) {
  ZSTD_CStream* const cstream = ZSTD_createCStream();
  if (cstream == NULL) {
    std::cerr << "ZSTD_createCStream() error" << std::endl;
    exit(1);
  }

  size_t const initResult = ZSTD_initCStream(cstream, compressionLevel);
  if (ZSTD_isError(initResult)) {
    std::cerr << "ZSTD_initCStream() error: " << ZSTD_getErrorName(initResult) << std::endl;
    exit(1);
  }
  ...
  for (const auto& buffer : buffers) {
    ZSTD_inBuffer input = {buffer->data(), (size_t)buffer->size(), 0};

    while (input.pos < input.size) {
      output.dst = compressed.data() + compressed.size(); // current position in output buffer
      ...
      compressed.resize(compressed.size() + output.pos); // update size of compressed buffer
  ...
  output.dst = compressed.data() + compressed.size();
  output.size = compressed.capacity() - compressed.size();
  output.pos = 0;

  size_t const remaining = ZSTD_endStream(cstream, &output);
  if (remaining != 0) {
    std::cerr << "Not all data was flushed" << std::endl;
  }

  compressed.resize(compressed.size() + output.pos); // final size of compressed buffer
 ...
 public:
  ...
  void getRecordBatchReader(const std::string& inputFile, uint32_t compressBufferSize) {
    ...
    for (int i = 0; i < numColumns; ++i) {
      auto fieldType = schema_->field(i)->type()->id();
      if (fieldType == arrow::Type::INT64 || fieldType == arrow::Type::STRING) {
        std::cout << "Pick column " << i << " " << schema_->field(i)->type()->ToString() << std::endl;
 ...
 protected:
  void doCompress(
    ...
    for (auto _ : state) {
      ...
      //            decompress(ipcWriteOptions, payloads, uncompressedBufferSize, decompressTime);
      {
        //        compressFunction f = zstdCompressBuffers; // multiple input, one output
        //        compressFunction f = zstdCompressBuffer; // memcpy into one
        compressFunction f = lz4CompressBuffer; // memcpy into one
        TIME_NANO_START(compressTime);
        auto out = f(buffers, /* compressionLevel= */ 3);
        TIME_NANO_END(compressTime);
        compressedSize += out.size();
        uncompressedSize += std::accumulate(
            buffers.begin(), buffers.end(), 0, [](int64_t sum, const std::shared_ptr<arrow::Buffer>& buffer) {
              return sum + buffer->size();
            });
      }
    }
