  ...
  for (const auto& item : handles_) {
    ...
    if (getNumUdfSym) {
      ...
      for (auto i = 0; i < numUdf; ++i) {
        ...
        signatures_.insert(std::make_shared<UdfSignature>(
            entry.name, dataType, argTypes, entry.variableArity, entry.enableTypeConversion));
      }
      free(udfEntries);
    } else {
    ...
    if (getNumUdafSym) {
      ...
      for (auto i = 0; i < numUdaf; ++i) {
        ...
        signatures_.insert(std::make_shared<UdfSignature>(
            entry.name, dataType, argTypes, intermediateType, entry.variableArity, entry.enableTypeConversion));
      }
      free(udafEntries);
    } else {
