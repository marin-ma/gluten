...
// A unary dummy expression that simply returns its child.
...
// of an unsupported expression in Gluten.
...
object DummyExpression {
  ...
  val sparkDummyExpressionIdentifier = new FunctionIdentifier(SPARK_DUMMY_EXPRESSION)

  val veloxDummyExpressionIdentifier = new FunctionIdentifier(VELOX_DUMMY_EXPRESSION)

  def registerFunctions(registry: FunctionRegistry): Unit = {
    registry.registerFunction(
      sparkDummyExpressionIdentifier,
    ...
    registry.registerFunction(
      veloxDummyExpressionIdentifier,
  ...
  def unregisterFunctions(registry: FunctionRegistry): Unit = {
    registry.dropFunction(sparkDummyExpressionIdentifier)
    registry.dropFunction(veloxDummyExpressionIdentifier)
  }
}
